    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.mainCard, { backgroundColor: colors.background }]}>
          <Text style={[styles.mainTitle, { color: theme === 'light' ? '#1A3B5C' : colors.text }]}>{t('adminDashboard.title')}</Text>
          <View style={styles.carouselContainer}>
            <Image source={{ uri: MOCK_CAROUSEL_IMAGES[currentImageIndex] }} style={styles.carouselImage} />
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statBox, { backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.statLabel, { color: colors.textSecondary }]}>{t('adminDashboard.stats.monuments')}</Text><View style={styles.statValueRow}><Ionicons name="business" size={24} color={colors.primary} /><Text style={[styles.statNumber, { color: colors.text }]}>155</Text></View></View>
            <View style={[styles.statBox, { backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.statLabel, { color: colors.textSecondary }]}>{t('adminDashboard.stats.users')}</Text><View style={styles.statValueRow}><Ionicons name="people" size={24} color={colors.primary} /><Text style={[styles.statNumber, { color: colors.text }]}>18,500</Text></View></View>
          </View>
          <View style={styles.statsRow}><View style={[styles.statBox, { width: '48%', backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.statLabel, { color: colors.textSecondary }]}>{t('adminDashboard.stats.admins')}</Text><View style={styles.statValueRow}><Ionicons name="git-network-outline" size={24} color={colors.primary} /><Text style={[styles.statNumber, { color: colors.text }]}>12</Text></View></View></View>
        </View>
      </ScrollView>
      <View style={styles.bottomNav}>
        <TouchableOpacity style={styles.navItem}><Ionicons name="home" size={22} color="#FFFFFF" /><Text style={styles.navText}>Principal</Text></TouchableOpacity>
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => router.replace('/admin-users')}><Ionicons name="settings" size={22} color="#FFFFFF" /><Text style={styles.navText}>Gestión de{'\n'}Usuarios</Text></TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={() => alert('Gestión de Monumentos')}><Ionicons name="library" size={22} color="#FFFFFF" /><Text style={styles.navText}>Gestión de{'\n'}Monumentos</Text></TouchableOpacity>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingTop: 180, paddingBottom: 120 }, // <--- Padding aumentado
  ...
  bottomNav: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', backgroundColor: '#0A2342', height: 90, paddingBottom: 20, position: 'absolute', bottom: 0, width: '100%' }, // <--- Altura y padding ajustados
